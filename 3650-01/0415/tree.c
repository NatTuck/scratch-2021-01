...
tree_put(tree* tt, long kk, long vv)
{
    ...
    // Is the rank of the left child more than
    // one different than the rank of the right
    // child?
    ...
    if (dr > 1) {
        ...
        // New right child is old root
        root->kids[1] = tree_make_node(uu->key, uu->val);

        // New right->right subtree is old right subtree
        root->kids[1]->kids[1] = tree_inc_ref(uu->kids[1]);

        // new right->left subtree is old left->right
        tree* rlst = uu->kids[0]->kids[1];
        root->kids[1]->kids[0] = tree_inc_ref(rlst);
