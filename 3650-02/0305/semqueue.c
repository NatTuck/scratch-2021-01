...
typedef struct shq {
    ...
    unsigned int qii; // Output index
    unsigned int qjj; // Input index
    sem_t isem; // Space - How many free slots
    sem_t osem; // Items - How many full slots
} shq;
...
queue_put(int xx)
{
    ...
    // Atomic-fetch-add(addr, val) does these operations:
    //  - int temp = *addr
    //  - *addr = temp + val
    //  - return temp
    // It does them atomically, so nothing
    // happens between the steps. Either they
    // all happen before any concurrent event or
    // all happen after it.
...
main(int _ac, char* _av[])
{
    ...
    shared = mmap(
        0, 1024 * 1024, PROT_READ|PROT_WRITE,
